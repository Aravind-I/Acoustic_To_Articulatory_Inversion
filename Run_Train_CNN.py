...
#for ss in np.arange(0,len(Trainsubs)):
    ...
    #MFCCinput_P=TimeDistributed(Dense(200))(mdninput_Lstm)
    ...
    postCNNFeq1=Conv1D(nb_filter=OutDim, filter_length=5, activation='relu',padding='same')(mdninput_Lstm)#MFCCinput_P)
    postCNNFeqBN1=BatchNormalization(axis=-1)(postCNNFeq1)

    postCNNFeq2=Conv1D(nb_filter=OutDim, filter_length=5, activation='relu',padding='same')(postCNNFeqBN1)
    postCNNFeqBN2=BatchNormalization(axis=-1)(postCNNFeq2)

    postCNNFeq3=Conv1D(nb_filter=OutDim, filter_length=5, activation='relu',padding='same')(postCNNFeqBN2)
    postCNNFeqBN3=BatchNormalization(axis=-1)(postCNNFeq3)
    
    postCNNFeq4=Conv1D(nb_filter=OutDim, filter_length=5, activation='relu',padding='same')(postCNNFeqBN3)
    postCNNFeqBN4=BatchNormalization(axis=-1)(postCNNFeq4)

    postCNNFeq5=Conv1D(nb_filter=OutDim, filter_length=5, activation='relu',padding='same')(postCNNFeqBN4)
    postCNNFeqBN5=BatchNormalization(axis=-1)(postCNNFeq5)

    postCNNFeq6=Conv1D(nb_filter=OutDim, filter_length=5, activation='relu',padding='same')(postCNNFeqBN5)
    postCNNFeqBN6=BatchNormalization(axis=-1)(postCNNFeq6)
    
    output=TimeDistributed(Dense(12*3, activation='linear'))(postCNNFeqBN3)
    ...
    OutFileName='CNN_6H_64F_SD_AAI_'+'Batch'+ str(BatchSize)+'_'+Sub+'_LSTMunits_'+str(NoUnits)+'_'
    fName=OutFileName
    print('..fitting model')
